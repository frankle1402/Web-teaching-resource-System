...
传统教学资源以 PPT 为主，这是线下��堂通过投影仪或电视一体机播放的最佳载体。然而，随着移动学习、互联网教学平台、5G 和人工智能等新技术的普及，教学资源的形态正在发生根本性变革。

### 产品定位

本系统是**面向医卫类教师的新一代教学资源生成平台**。教师只需输入简单的教学信息，系统即可通过 AI 快速生成基于 HTML5 的可交互、自适应教学资源，取代传统 PPT 成为更适合数字化教学的资源形态。

### HTML5 教学资源的核心优势

| 维度 | PPT | HTML5 教学资源 |
|------|-----|----------------|
| 跨平台 | 需要安装软件 | 浏览器原生支持，PC/平板/手机自适应 |
| 交互性 | 有限 | 原生支持测验、动画、即时反馈 |
| 传播性 | 文件传输 | URL 分享、iframe 嵌入、搜索引擎可索引 |
| 可维护性 | 手动修改 | AI 可读可生成，快速迭代更新 |

### 开发原则

所有功能设计和技术决策必须围绕以下原则：

1. **降低门槛**：让医卫类教师无需技术背景即可创建专业教学资源
2. **AI 优先**：技术选型优先考虑 AI 生成友好性，输出保持 HTML5 标准格式
3. **互联网原生**：确保资源便于传播、检索、嵌入和分享

### 跨平��兼容策略

生成的 HTML5 资源支持三种访问方式：

| 方式 | 说明 | 适用场景 |
|------|------|----------|
| 本平台在线访问 | 通过 `/r/:uuid` 公开链接 | 日常使用 |
| 超链接跳转 | 从 Moodle 等外部平台跳转 | LMS 集成 |
| 离线打包 | ZIP 包上传到 Moodle | 离线教学 |

**样式隔离**：所有 CSS 选择器添加 `.tr-resource-container` 命名空间前缀，确保在任何环境下都能正常显示。

**CDN 策略**：使用 jsDelivr（国内有节点，安全可靠）
```
...
支持教师从"一键生成"到"深度定制"的完整光谱：

| 模式 | 用户输入 | 流程 | 适用场景 |
|------|----------|------|----------|
| **快速模式** | 主题 + 基础信息 | 一步直接生成 | 时间紧迫、初次体验 |
| **引导模式** | 选择内容方向/模板 | 引导式生成 | 有一定想法但不清晰 |
| **专家模式** | 上传教案/自定义提示词 | 大纲编辑→分步生成 | 有明确教学设计需求 |

### 教学法大纲模板

系统内置 5 种医卫类教育常用教学法模板：

1. **CBL 案例教学法**：案例导入→问题提出→知识讲解→处理方案→案例延伸
2. **操作技能四步法**：目的意义→用物准备→操作步骤→要点与常见错误
3. **PBL 问题导向教学法**：问题情境→问题分解→知识探索→方案制定→反思总结
4. **翻转课堂教学法**：学习目标→核心概念→重点难点→课堂讨论→拓展思考
5. **循证护理/医学教学法**：临床问题(PICO)→证据检索→证据评价→证据应用→效果评价
6. **AI 智能生成**：根据上传内容自动匹配最适合的大纲结构
