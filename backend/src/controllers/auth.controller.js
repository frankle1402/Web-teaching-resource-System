...
class AuthController {
  ...
  async completeProfile(req, res) {
    try {
      const { real_name, organization, nickname, avatar_url } = req.body;

      // 验证必填字段
      if (!real_name || !organization) {
        return res.status(400).json({
          success: false,
          error: {
            code: 'MISSING_REQUIRED_FIELDS',
            message: '请填写真实姓名和单位/机构'
          }
        });
      }
      ...
      // 更新用户资料
      const updateFields = ['real_name = ?', 'organization = ?', 'profile_completed = 1'];
      const updateParams = [real_name, organization];
